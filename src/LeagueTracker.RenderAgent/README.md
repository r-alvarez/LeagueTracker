...
2. Downloads the job's `.rofl`, launches the game client in replay mode.
...
4. Run `LeagueTracker.RenderAgent.exe` in a terminal. First run auto-detects
   the League install and adds `EnableReplayApi=1` to `Config/game.cfg` if
   missing (the Replay API needs it; the game reads it at launch).

For always-on operation, register it with Task Scheduler (run at logon; it
must run in the interactive session - the game has to render to a real
desktop for capture to work).
