...
public static class YouTubeChapters
{
    ...
    public static string? Describe(ReviewReel reel, IReadOnlyList<(double VideoSec, double GameSec)> clockMap, string matchId, string? reviewUrl)
    {
        ...
        var lines = new List<string> { $"Match {matchId}" };
    ...
    // The same map the tracker's page seeks with: a capture restart leaves the
