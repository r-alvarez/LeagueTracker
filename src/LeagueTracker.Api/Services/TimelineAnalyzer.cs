...
public static class TimelineAnalyzer
{
    ...
    public static TimelineAnalysis Analyze(string timelineRaw, MatchInfoDto info, MatchParticipantDto me)
    {
        ...
        // Every 3 minutes (the item race wants density) plus the milestone
        // minutes the lane-diff table is built around, out to 90 for the
        // marathon games - minutes past the last frame skip automatically.
        var laneDiffs = new List<LaneDiffPoint>();
        foreach (var minute in Enumerable.Range(1, 30).Select(i => i * 3)
            .Concat([10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 90]).Distinct().Order())
        {
        ...
        HashSet<int> allyPids, HashSet<int> enemyPids, int myPid)
    {
        ...
        var cameraPid = Survivor(Ally, allowRecentlyDead: false)
            ?? Survivor(enemyPids.Contains, allowRecentlyDead: false)
            ?? Survivor(Ally, allowRecentlyDead: true)
            ?? Survivor(enemyPids.Contains, allowRecentlyDead: true)
